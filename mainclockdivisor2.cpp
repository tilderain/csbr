...
int timeRemaining = 0;
	...
	while(game.running && game.switchstage.mapno < 0)
	{
		...
		if(appTime >= nextTick || game.ffwdtime)
		{
			nextTick = appTime + 0.016667;//(settings->game_fps ? 17 : 20);
			timeRemaining = 16667;
			run_tick();
		...
		else
		{
			...
			if (timeRemaining) {
				...
				timeRemaining = 0; //take it out or leave it in??
			}
		}
	}
}
