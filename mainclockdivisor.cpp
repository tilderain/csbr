...
int32_t nexttick = 0;
	...
	while(game.running && game.switchstage.mapno < 0)
	{
		...
		stat("frameTime = f %f", frameTime);
		stat("appTime = f %f", appTime);
		
		// get time until next tick
		int32_t curtime = SDL_GetTicks();
		int32_t timeRemaining = 100;
		
		int curTick = appTime/30;
		
		stat("curTick = %d", curTick);
		stat("lastTick = %d", lastTick);
		
		if(curTick != lastTick || game.ffwdtime){
			...
			//nexttick = curtime + (settings->game_fps ? 17 : 20);
		...
		else
		{
			...
			timeRemaining /= 2;
			if (timeRemaining) {
				stat("being delayed for this much %d", timeRemaining);
				SDL_Delay(timeRemaining);
				stat("been delayed for this much %d", timeRemaining);
			}
		}
	}
}
