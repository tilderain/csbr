//you know, there's probably a better way to do this
//but i don't know it
//so you can crucify me for the abominations contained within this file
//- tilderain, 2016
...
bool error = false;
	...
	if (SCALE >= 1)
	{
		stat("fonts: using bitmapped from");
		
		SDL_Surface *sheet = SDL_LoadBMP(fontfilelist[(SCALE - 1)]);
		if (!sheet)
		{
			staterr("Couldn't open bitmap font file:");
			return 1;
		}
...
int x, y, i;
	...
	for(i=0;bitmap_map[i];i++)
	{
		...
		letter = SDL_CreateRGBSurface(SDL_SRCCOLORKEY, \
							(BITMAP_CHAR_WIDTH * SCALE)+(1 * SCALE), (BITMAP_CHAR_HEIGHT * SCALE)+(1 * SCALE),
							format->BitsPerPixel, \
		...
		srcrect.x = x;
		srcrect.y = y;
		srcrect.w = (BITMAP_CHAR_WIDTH * SCALE);
		srcrect.h = (BITMAP_CHAR_HEIGHT * SCALE);
		...
		x += (BITMAP_SPAC_WIDTH * SCALE);
		if (x >= sheet->w)
		{
			x = 0;
			y += (BITMAP_SPAC_HEIGHT * SCALE);
		}
	}
	...
	for(int i=0;i<NUM_FONT_LETTERS;i++)
	{
		if (fgfont.letters[i])
		{
			letters[i] = SDL_CreateRGBSurface(SDL_SRCCOLORKEY, \
							(BITMAP_CHAR_WIDTH * SCALE) + (1 * SCALE), (BITMAP_CHAR_HEIGHT * SCALE) + \
							(1 * SCALE) +(SHADOW_OFFSET * SCALE),
							format->BitsPerPixel, \
							format->Rmask, format->Gmask,
							format->Bmask, format->Amask);
