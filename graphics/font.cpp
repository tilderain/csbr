...
#define BIGSHADOW_OFFSET		2		// distance of drop shadows

#define BITMAP_CHAR_WIDTH	5		// width of each char
#define BITMAP_CHAR_HEIGHT	12		// height of each char
#define BITMAP_SPAC_WIDTH	6		// how far apart chars are from each other on the sheet
#define BITMAP_SPAC_HEIGHT	12		// ditto for Y-spacing

#define BIGBITMAP_CHAR_WIDTH	10		// width of each char
#define BIGBITMAP_CHAR_HEIGHT	24		// height of each char
#define BIGBITMAP_SPAC_WIDTH	12		// how far apart chars are from each other on the sheet
#define BIGBITMAP_SPAC_HEIGHT	24		// ditto for Y-spacing
...
const char *bigbmpfontfile = "bigfont.bmp";
...
bool error = false;
	...
	if (SCALE >= 2)
		{
		stat("fonts: using bitmapped from %s", bigbmpfontfile);
		
		SDL_Surface *sheet = SDL_LoadBMP(bigbmpfontfile);
		if (!sheet)
		{
			staterr("Couldn't open bitmap font file: '%s'", bigbmpfontfile);
			return 1;
		}
		
		uint32_t fgindex = SDL_MapRGB(sheet->format, 255, 255, 255);
		
		error |= whitefont.InitBigBitmapChars(sheet, fgindex, 0xffffff);
		error |= greenfont.InitBigBitmapChars(sheet, fgindex, 0x00ff80);
		error |= bluefont.InitBigBitmapChars(sheet, fgindex, 0xa0b5de);
		error |= shadowfont.InitBigBitmapCharsShadowed(sheet, fgindex, 0xffffff, 0x5f90ca);
	}
	#ifdef CONFIG_ENABLE_TTF
	else
	{
...
bool NXFont::InitBigBitmapChars(SDL_Surface *sheet, uint32_t fgcolor, uint32_t color)
{
SDL_PixelFormat *format = sdl_screen->format;
SDL_Rect srcrect, dstrect;
SDL_Surface *letter;
int x, y, i;
	...
	for(i=0;bitmap_map[i];i++)
	{
		...
		// is some space between letters in autospaced text such as on the menus.
		letter = SDL_CreateRGBSurface(SDL_SRCCOLORKEY, \
							BIGBITMAP_CHAR_WIDTH+2, BIGBITMAP_CHAR_HEIGHT+2,
							format->BitsPerPixel, \
							format->Rmask, format->Gmask,
							format->Bmask, format->Amask);
		if (!letter)
		...
		srcrect.x = x;
		srcrect.y = y;
		srcrect.w = BIGBITMAP_CHAR_WIDTH;
		srcrect.h = BIGBITMAP_CHAR_HEIGHT;
		...
		// advance to next position on sheet
		x += BIGBITMAP_SPAC_WIDTH;
		if (x >= sheet->w)
		{
			x = 0;
			y += BIGBITMAP_SPAC_HEIGHT;
		}
	}
...
bool NXFont::InitBigBitmapCharsShadowed(SDL_Surface *sheet, uint32_t fgcolor, \
									uint32_t color, uint32_t shadowcolor)
{
SDL_PixelFormat *format = sdl_screen->format;
NXFont fgfont, shadowfont;
SDL_Rect dstrect;
	...
	// create temporary fonts in the fg and shadow color
	if (fgfont.InitBigBitmapChars(sheet, fgcolor, color))
		return 1;
	
	if (shadowfont.InitBigBitmapChars(sheet, fgcolor, shadowcolor))
		return 1;
	...
	for(int i=0;i<NUM_FONT_LETTERS;i++)
	{
		...
		{
			letters[i] = SDL_CreateRGBSurface(SDL_SRCCOLORKEY, \
							BIGBITMAP_CHAR_WIDTH+2, BIGBITMAP_CHAR_HEIGHT+2+BIGSHADOW_OFFSET,
							format->BitsPerPixel, \
							format->Rmask, format->Gmask,
							format->Bmask, format->Amask);
